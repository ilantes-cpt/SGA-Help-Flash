/*
page 50302 "Sel. Contenido Contenedor Diar"
{
    ApplicationArea = All;
    Caption = 'Selector Content Container Journal';
    PageType = List;
    ...
    begin
        ...
        if Rec.generado then begin
            ...
            rcontenido.SetRange("Libro registro productos", cDocumento);
                    ...
                    case rec.Tipo of
                        rConf."Tipo pallet":
                            begin
                                ...
                                if rContenido.find('-') then
                                    repeat
                                        rContenido.Validate("Libro registro productos", cDocumento);
                                        rContenido.Validate("Sección registro productos", cSeccion);
    ...
    procedure EstablecerDoc(cDoc: code[20]; cSeccn: code[20]; iLin: Integer; dCantBase: Decimal; cAlm: code[10]; cProd: code[20])
    var
    ...
    begin
        ...
        rec.setfilter(FiltroNombreDiario, '<>%1', cdoc);
        rec.setfilter(FiltroSeccionDiario, '<>%1', cSeccn);
        rec.setfilter(FiltroLineaDiario, '<>%1', iLin);
        ...
        rContenido.reset;
        rcontenido.SetRange("Libro registro productos", cdoc);
        rcontenido.SetRange("Sección registro productos", cSeccn);
        ...
        if rContenido.find('-') then
            repeat
                ...
                rcontpadre.setrange("Libro registro productos", cdocumento);
                rcontpadre.setrange("Sección registro productos", cSeccion);
                rcontpadre.SetRange(LinDiario, ilin);
                ...
                if rContPadre.findset then
                    if rcontpadre.calcsums(cantidad) then
                        if rcontpadre.cantidad = dpalet then
                            if InsertarRegistro(rconf."Tipo pallet", rcontenido.padre, dpalet) then bGenerado := true;
                    ...
                    if rContPadre.findset then begin
                        if rcontpadre.calcsums(cantidad) then
                            if rcontpadre.cantidad = dcaja then
                                if InsertarRegistro(rconf."Tipo caja", rContPadre."Código", dcaja) then bgenerado := true;
                    end;
                end;
                if not bgenerado then begin
                    ...
                    if not rec.findset then begin
                        ...
                        if not rec.findset then begin
                            if InsertarRegistro('', rcontenido.IMEI, 1) then bgenerado := true;
            ...
            case rec.tipo of
                rconf."Tipo caja":
                    begin
                        ...
                        //rContenedor.SetRange(ContieneProductoFiltradoCajaDi, true);
                        rContenedor.SetFilter(FiltroNombreDiario, '<>%1', cDocumento);
                        rContenedor.SetFilter(FiltroSeccionDiario, '<>%1', cSeccion);
                        rContenedor.SetFilter(FiltroLineaDiario, '<>%1', iLinea);
                        ...
                        if rContenedor.find('-') then begin
                            ...
                            pContenedor.EstablecerFiltrosSeleccionDiario(cAlmacen, cProducto, cDocumento, cSeccion, iLinea);
                ...
                rconf."Tipo pallet":
                    begin
                        ...
                        rcontenedor.SetRange(FiltroProducto, cProducto);
                        //rContenedor.SetRange(ContieneProductoFiltPalletDi, true);
                        rContenedor.SetFilter(FiltroNombreDiario, '<>%1', cDocumento);
                        rContenedor.SetFilter(FiltroSeccionDiario, '<>%1', cSeccion);
                        rContenedor.SetFilter(FiltroLineaDiario, '<>%1', iLinea);
                        rContenedor.SetFilter(FiltroPedidoEnvio, '%1', cDoc2);
                ...
                '':
                    begin
                        ...
                        rcontenido.SetFilter("Libro registro productos", '<>%1&%2', cDocumento, cDoc2);
                        rcontenido.SetFilter("Sección registro productos", '<>%1', cSeccion);
                        rcontenido.SetFilter(LinDiario, '<>%1', iLinea);
                        rcontenido.SetRange(PedTrans, '');
                        rcontenido.SetRange(LinPedTrans, 0);
                        rContenido.SetRange(PedVenta, '');
                        rContenido.SetRange(LinPedVenta, 0);
    ...
    begin
        ...
        rContenido.SetRange("Cód Almacén", cAlmacen);
        rcontenido.SetRange("Nº producto", cProducto);
        rcontenido.SetFilter("Libro registro productos", '<>%1', cDocumento);
        rcontenido.SetFilter("Sección registro productos", '<>%1', cSeccion);
        rcontenido.SetFilter(LinDiario, '<>%1', iLinea);
        rcontenido.SetRange(PedVenta, '');
        rcontenido.SetRange(LinPedVenta, 0);
