/*
page 50309 "Sel. Contenido Contenedor Rcp"
{
    ApplicationArea = All;
    Caption = 'Selector Content Container Rcpt';
    PageType = List;
    ...
    begin
        ...
        rec.setfilter(FiltroPedidoRecep, '<>%1', cdoc);
        rec.setfilter(FiltroLineaRecep, '<>%1', iLin);
        ...
        if rContenido.find('-') then
            repeat
                ...
                rcontpadre.setrange(RecepAlm, cdocumento);
                rcontpadre.setrange(LinRecep, ilinea);
            ...
            case rec.tipo of
                rconf."Tipo caja":
                    begin
                        ...
                        //rContenedor.SetRange(ContieneProductoFiltradoCajaRc, true);
                        rContenedor.SetFilter(FiltroPedidoRecep, '<>%1', cDocumento);
                        rContenedor.SetFilter(FiltroLineaRecep, '<>%1', iLinea);
                ...
                rconf."Tipo pallet":
                    begin
                        ...
                        rcontenedor.SetRange(FiltroProducto, cProducto);
                        //rContenedor.SetRange(ContieneProductoFiltPalletRc, true);
                        rContenedor.SetFilter(FiltroPedidoRecep, '<>%1', cDocumento);
                        rContenedor.SetFilter(FiltroLineaRecep, '<>%1', iLinea);
                ...
                '':
                    begin
                        ...
                        rcontenido.SetFilter(RecepAlm, '<>%1&%2', cDocumento, cDoc2);
                        rcontenido.SetFilter(LinRecep, '<>%1', iLinea);
                        rcontenido.SetRange(PedTrans, '');
                        rcontenido.SetRange(LinPedTrans, 0);
                        rcontenido.SetRange(PedVenta, '');
                        rcontenido.SetRange(LinPedVenta, 0);
    ...
    begin
        ...
        rContenido.SetRange("Cód Almacén", cAlmacen);
        rcontenido.SetRange("Nº producto", cProducto);
        rcontenido.SetFilter(RecepAlm, '<>%1', cDocumento);
        rcontenido.SetFilter(LinRecep, '<>%1', iLinea);
        rconf.get;
        case rec.tipo of
            rconf."Tipo caja":
